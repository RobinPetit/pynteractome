...
unhandled_genes = set()
with open(DATA_DIR + 'gene2omim.tsv') as diseage_genes_file:
...
with open(DATA_DIR + 'converted gene2omim.tsv', 'w') as new_disease_genes_file:
    new_disease_genes_file.write(''.join(file_header))
    new_disease_genes_file.writelines(new_file_content)
with open(DATA_DIR + 'unhandled genes (gene2omim).txt', 'w') as unhandled_genes_file:
